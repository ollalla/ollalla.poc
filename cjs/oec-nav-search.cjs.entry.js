...
const NavSearch = class {
  ...
  connectedCallback() {
    this.key$.pipe(index$1.debounce((key) => key === 'Enter' ? index$1.timer(0) : index$1.timer(3000)), index$1.switchMap(() => index$1.of((this.searchInput.value || '').trim())), index$1.filter(x => !!x && x.length > 3), // greater than 3 characters
    index$1.distinctUntilChanged(), index$1.takeUntil(this.destroy$)).subscribe(() => {
